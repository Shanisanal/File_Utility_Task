...
bool WriteSrecCountRecord(FILE* pOutputFile, uint32_t ulRecordCount)
{
    ...
    {
        fprintf(stderr, "Error: Record count exceeds maximum for S5/S6 records\n");
        return blCountRecordSuccess;
    }
...
bool WriteSrecTerminationRecord(FILE *pOutputFile)
{
    ...
    if (pOutputFile != NULL)
    {
        ...
        ucTerminationByteCount = (uint8_t)(SREC_DATA_RECORD_ADDR_SIZE  + SREC_CHECKSUM_SIZE);
        ucTerminationChecksum = CalculateSrecChecksum(ucTerminationByteCount, SREC_DATA_RECORD_ADDR, NULL, 0);

        /* Termination record format: [Type][Byte Count][Start Address][Checksum] */
